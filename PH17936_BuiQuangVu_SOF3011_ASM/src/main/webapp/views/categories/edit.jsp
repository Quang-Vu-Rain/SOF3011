...
<div class="container">
	<div
		...
		<hr>
		<form method="POST"
			action="/PH17936_BuiQuangVu_Lab5_300322/categories/update?id=${ cate.id }">
			<div class="mb-3">
				<label class="form-label fw-bold">Tên Category</label> <input
					type="text" name="ten" class="form-control" value="${ cate.ten }">
			</div>
			...
			<div class="mb-3">
				<select class="form-select" aria-label="Default select example"
					...
					<c:forEach items="${ dsUser }" var="user">
						<option value="${ user.id }" ${ cate.user.id == user.id ? "selected" : "" }>${ user.hoTen }</option>
					</c:forEach>
				</select>
			</div>
